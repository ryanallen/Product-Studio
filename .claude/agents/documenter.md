...
9. **End of job – file review:** When the main doc work is done, run a closing pass:
   - Use [research](../skills/research/SKILL.md) (or a systematic read) to learn what every relevant file is: scope = files you read or wrote this run, or the set the task asked to refine. For each file: name, path/location, brief content summary (what it is and what it does).
   ...
   - Go through each file and check whether anything needs updating to support the refinement (cross-refs, consistency with other docs, paths, wording). If yes, make the update or note it.
